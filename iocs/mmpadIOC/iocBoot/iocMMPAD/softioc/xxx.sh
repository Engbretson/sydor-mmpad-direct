...
# Manually set IOC_STARTUP_DIR if 29idcARV5.sh will reside somewhere other than softioc
#!IOC_STARTUP_DIR=/home/username/epics/ioc/synApps/29idcARV5/iocBoot/ioc29idcARV5/softioc

# Manually set IOC_COMMAND_DIR if 29idcARV5.sh will reside somewhere other than softioc
#!IOC_COMMAND_DIR=/home/username/epics/ioc/synApps/29idcARV5/iocBoot/ioc29idcARV5/softioc/commands
...
IOC_BINARY=mmpadDetectorApp
#IOC_BINARY=29idcARV5
# The absolute path to the directory containing the IOC's bin directory, if running an example IOC binary
#!IOC_BIN_DIR=/home/username/epics/synApps/support/29idcARV5/bin
...
if [ ${#IOC_BINARY} -gt 15 ]
then
    IOC_BINARY1=${IOC_BINARY:0:15}
fi

    MY_UID=`${ID} -u`
    # The '\$' is needed in the pgrep pattern to select 29idcARV5, but not 29idcARV5.sh
    IOC_PID=`${PGREP} ${IOC_BINARY1}\$ -u ${MY_UID}`
